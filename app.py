...
@st.cache_data
def run_stl(aqi_values, dates):
    s = pd.Series(aqi_values, index=dates)
    stl = STL(s, period=365, robust=True)
    res = stl.fit()
    return res.trend, res.seasonal, res.resid

@st.cache_data
def run_arima(aqi_values, dates, d_val):
    s = pd.Series(aqi_values, index=dates)
    train = s[:-30]
    test  = s[-30:]
    results = []
...
@st.cache_data
def get_forecast(aqi_values, dates, order, steps):
    s = pd.Series(aqi_values, index=dates)
...
@st.cache_data
def run_markov(aqi_values, dates):
    s = pd.Series(aqi_values, index=dates)
...
with tab1:
    ...
    with st.spinner("Running STL decomposition..."):
        trend, seasonal, resid = run_stl(aqi_vals, aqi_dates)
...
with tab4:
    ...
    with st.spinner("Running ARIMA grid search (p,d,q) ∈ {0..3}³ ..."):
        best_order, results_df = run_arima(aqi_vals, aqi_dates, d_val)
    ...
    with st.spinner("Generating forecast..."):
        fc_mean, fc_ci, residuals = get_forecast(
            aqi_vals, aqi_dates, best_order, forecast_days)
...
with tab5:
    ...
    P, pi, STATES, counts = run_markov(aqi_vals, aqi_dates)
